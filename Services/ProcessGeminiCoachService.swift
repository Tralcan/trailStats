...
class ProcessGeminiCoachService {
    ...
    func getProcessRaceEstimation(for process: TrainingProcess, with activities: [Activity], completion: @escaping (Result<RaceProjection, Error>) -> Void) {
        ...
        var maxRPE: Double = 0.0
        for activity in activities {
            if let rpe = activity.rpe {
                maxRPE = max(maxRPE, rpe)
            }
        }

        if maxRPE >= 7.0 {
            kpis["max_rpe"] = String(format: "%.0f", maxRPE)
        }
        
        GeminiCoachService.fetchObservation(activityId: process.id.uuidString, kpis: kpis, cacheManager: self.cacheManager) { result in
